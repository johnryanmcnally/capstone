...
# test_title = st.text_input('Enter A Dish Name (or "x" to quit): ')


def predict_ingredients(test_title):
    doc = nlp(test_title)
    X_test = [0 for x in range(1000)]
    for token in doc:
        if token.pos_ not in ['NUM', 'PUNCT']:
            if token.lower_ not in TITLE_STOPWORDS:
                if token.lower_ in title_toks_keys:
                    X_test[title_toks_keys.index(token.lower_) + 1] = 1
                else:
                    X_test[0] = 1
    ...
    ings = []
    dups = []
    for i in (-pred[0]).argsort():
        if pred[0][i] >= thresh:
            if i == 0:
                ings.append('Other')
            else:
                ings.append(ing_keys[i-1])
        else:
            break
    # Filter some duplicates by taking more basic ingredient (flour, all purpose flour --> flour)
    for ing in ings:
        if len(ing.split()) == 1:
            # Single word ingredient (e.g. flour, butter)
            for other in ings:
                if ing in other:
                    if ing != other:
                        dups.append(other)
...
# if test_title:
#     ings = predict_ingredients(test_title)
#     for ing in ings:
#         st.write(ing)